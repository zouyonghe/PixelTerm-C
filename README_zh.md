...
更多命令示例和参数说明见 [USAGE_zh.md](USAGE_zh.md)。
...
- 终端与协议说明见 [docs/TERMINAL_PROTOCOL_SUPPORT.md](docs/TERMINAL_PROTOCOL_SUPPORT.md)。

## 配置

如果存在 `$XDG_CONFIG_HOME/pixelterm/config.ini`，PixelTerm-C 会自动读取；也可以通过 `--config` 指定其他配置文件。建议先从 [`config.example.ini`](config.example.ini) 开始，用 `[default]` 保存通用设置，再按终端环境添加覆盖分组，匹配 `TERM_PROGRAM`、`LC_TERMINAL`、`TERMINAL_NAME` 或 `TERM`。
...
- 使用方法与命令行选项：[USAGE_zh.md](USAGE_zh.md)
- 键盘和鼠标操作：[CONTROLS_zh.md](CONTROLS_zh.md)
- 更新记录：[CHANGELOG.md](CHANGELOG.md)
- 终端与协议说明：[docs/TERMINAL_PROTOCOL_SUPPORT.md](docs/TERMINAL_PROTOCOL_SUPPORT.md)
