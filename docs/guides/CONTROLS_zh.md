# 🎮 控制

*[English](CONTROLS.md) | 中文 | [日本語](CONTROLS_ja.md)*

### 全局控制

| 按键 | 功能 |
|-----|----------|
| ESC | 退出应用程序 |
| Ctrl+C | 强制退出 |
| ? | 显示完整快捷键帮助；按任意键或鼠标按钮关闭 |

### 鼠标控制

鼠标交互显著增强了不同模式下的导航和选择体验。

| 操作 | 功能 | 适用模式 | 说明 |
|---------|----------|------------------|-------|
| 左键单击 | 切换到下一张图像 | 图像视图（单图模式） | 视频时为播放/暂停。 |
| 左键双击 | 切换到网格预览 | 图像视图（单图模式） | |
| 鼠标滚轮上/下 | 缩放图像（鼠标在图像区域） | 图像视图（单图模式） | 视频/动图不支持；不在图像区域时忽略。 |
| 左键单击 | 选中图像 | 网格预览 | 选中光标下的图像。 |
| 左键双击 | 在图像视图中打开选中图像 | 网格预览 | 打开光标位置的图像。 |
| 鼠标滚轮上/下 | 上/下翻页 | 网格预览 | 翻页浏览图像。 |
| 左键单击 | 选中页 | 电子书预览 | 选中光标下的页。 |
| 左键双击 | 打开选中页 | 电子书预览 | 在阅读模式打开该页。 |
| 鼠标滚轮上/下 | 上/下翻页 | 电子书预览 | 翻页浏览页面。 |
| 左键单击 | 下一页 | 电子书阅读 | 根据布局前进 1 或 2 页。 |
| 左键双击 | 切换到电子书预览 | 电子书阅读 | |
| 鼠标滚轮上/下 | 上一页/下一页 | 电子书阅读 | |
| 左键单击 | 选中条目 | 文件管理器模式 | 选中光标下的文件或目录。 |
| 左键双击 | 打开选中条目（目录/文件） | 文件管理器模式 | 进入目录或打开图像。 |
| 鼠标滚轮上/下 | 向上/向下导航条目 | 文件管理器模式 | 滚动文件和目录列表。 |

### 图像视图（单图模式）

这是查看单张图像时的默认模式。

| 按键 | 功能 |
|-----|----------|
| ←/→ | 上一张/下一张图像 |
| ↑/↓ | 上一张/下一张图像 |
| h/k | Vim风格导航（上一张图像） |
| l/j | Vim风格导航（下一张图像） |
| Enter | 切换进入网格预览模式 |
| TAB | 在图像视图 / 网格预览 / 文件管理器间循环切换 |
| i | 切换图像信息显示 |
| `~` / `` ` `` | 切换Zen模式（隐藏/显示所有文字信息） |
| r | 删除当前图像 |
| d/D | 切换抖动开/关 |

### 视频播放

当图像视图中当前项是视频时，可使用以下快捷键。

| 按键 | 功能 |
|-----|----------|
| 左键单击 | 播放/暂停 |
| 空格 | 播放/暂停 |
| ←/→ | 快退/快进 5 秒 |
| ↑/↓ | 切换上一个/下一个媒体 |
| F | 显示/隐藏 FPS |
| + / - | 调整视频缩放（`+` 放大，`-` 缩小） |
| p / P | 切换视频协议：`text -> sixel -> iterm2 -> kitty -> text` |

### 网格预览（缩略图模式）

此模式以网格形式显示多张图像缩略图。

| 按键 | 功能 |
|-----|----------|
| ←/→ | 移动选中项（左/右） |
| ↑/↓ | 移动选中项（上/下） |
| h/j/k/l | Vim风格导航（左/下/上/右） |
| PgUp/PgDn | 在网格中翻页 |
| Enter | 在图像视图中打开选中图像 |
| TAB | 在图像视图 / 网格预览 / 文件管理器间循环切换 |
| `~` / `` ` `` | 切换Zen模式（隐藏/显示所有文字信息） |
| r | 删除选中图像 |
| d/D | 切换抖动开/关 |
| +/= | 放大 |
| - | 缩小 |

### 电子书阅读

| 按键 | 功能 |
|-----|----------|
| ←/→ | 上一页/下一页 |
| ↑/↓ | 上/下翻页（单页：1；双页：2） |
| h/j/k/l | Vim风格导航（左/下/上/右） |
| PgUp/PgDn | 上/下翻页 |
| P | 跳转页码（输入数字，回车跳转，P/ESC取消） |
| Enter | 切换到电子书预览 |
| TAB | 返回文件管理器 |
| `~` / `` ` `` | 切换Zen模式（隐藏/显示所有文字信息） |

说明：阅读模式会根据终端宽高比自动选择单页/双页布局。

### 电子书预览（页面网格）

| 按键 | 功能 |
|-----|----------|
| ←/→ | 左/右移动选中项 |
| ↑/↓ | 上/下移动选中项 |
| h/j/k/l | Vim风格导航（左/下/上/右） |
| PgUp/PgDn | 在网格中翻页 |
| P | 跳转页码（输入数字，回车跳转，P/ESC取消） |
| Enter | 在阅读模式打开选中页 |
| TAB | 切换到阅读模式打开选中页 |
| +/= | 放大 |
| - | 缩小 |

### 文件管理器模式

此模式允许浏览目录和文件。英文字母键用于按首字母快速跳转文件条目。

| 按键 | 功能 |
|-----|----------|
| ←/→ | 进入父目录 / 打开选中目录/文件 |
| ↑/↓ | 向上/向下导航条目 |
| Enter | 打开选中目录或文件 |
| TAB | 打开图像预览/电子书预览或返回 |
| Backspace | 显示/隐藏隐藏文件 |
| 任意字母键 (a-z/A-Z) | 跳转到以该字母开头的下一个条目 |
