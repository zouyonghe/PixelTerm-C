...
static gint app_book_preview_page_virtual_row_offset(const PreviewLayout *layout, gint scroll) {
    if (!layout || layout->rows <= 0) {
        return 0;
    }
...
ErrorCode app_book_preview_page_move(PixelTermApp *app, gint direction) {
    ...
    gint current_page_virtual_row_offset = app_book_preview_page_virtual_row_offset(&layout, app->book.preview_scroll);
    ...
    gint page_row_position = current_row - app->book.preview_scroll + current_page_virtual_row_offset;
    ...
    gint new_page_virtual_row_offset = app_book_preview_page_virtual_row_offset(&layout, new_scroll);
...
ErrorCode app_book_preview_scroll_pages(PixelTermApp *app, gint direction) {
    ...
    return app_book_preview_page_move(app, direction > 0 ? 1 : -1);
}
